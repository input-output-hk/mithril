...
/// Service that synchronizes the certificate chain with an remote aggregator
...
mod tests {
    ...
    mod should_sync {
        ...
        async fn should_sync_if_force_true_without_checking_genesis_certificate() {
            let synchroniser = mocked_synchroniser!(with_remote_genesis: Err(CertificateRetrieverError(anyhow!(
        ...
        async fn should_abort_with_error_if_force_false_and_fails_to_retrieve_remote_genesis() {
            let synchroniser = mocked_synchroniser!(
                with_remote_genesis: Err(CertificateRetrieverError(anyhow!("failure"))),
