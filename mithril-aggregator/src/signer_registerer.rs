...
/// Implementation of s slave [SignerRegisterer]
    ...
    async fn synchronize_signers(
        ...
        for signer in signers {
            ...
            if self
                ...
                .map_err(|e| SignerRegistrationError::StoreError(anyhow!(e)))?.is_some()
            {
                return Err(SignerRegistrationError::ExistingSigner(
                    Box::new(
                    signer_with_stake,
                )))
            }
...
mod tests {
    ...
    mod slave {
        use crate::signer_registerer::MithrilSignerRegistererSlave;
