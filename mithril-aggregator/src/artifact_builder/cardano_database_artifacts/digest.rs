...
impl DigestArtifactBuilder {
    ...
    fn create_archive_file(&self, beacon: &CardanoDbBeacon) -> Result<PathBuf, anyhow::Error> {
        ...
        let digest_file_name = Self::get_digests_file_name(&self.network, beacon, "json");
        self.snapshotter
            .snapshot_subset(
                &digest_archive_file_path,
                vec![PathBuf::from(digest_file_name)],
            )
            .with_context(|| {
                format!(
