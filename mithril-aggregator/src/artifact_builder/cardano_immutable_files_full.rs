...
use crate::{
    snapshot_uploaders::SnapshotLocation, snapshotter::OngoingSnapshot, CompressionFormat,
    SnapshotError, SnapshotUploader, Snapshotter,
};
    ...
    async fn create_snapshot_archive(
        ...
        let ongoing_snapshot =
            tokio::task::spawn_blocking(move || -> Result<OngoingSnapshot, SnapshotError> {
                snapshotter.snapshot(&snapshot_name, CompressionFormat::Gunzip)
            })
            .await??;
