...
When deploying the infrastructure, the version of the Cardano node is matched exactly and fails over the minor version by matching the folder name in the `mithril-infra/assets/docker/cardano/config` folder. If the new version configuration files are valid for all minor versions you can name the folder by ommitting the patch version (eg `10.7` instead of `10.7.1`).
