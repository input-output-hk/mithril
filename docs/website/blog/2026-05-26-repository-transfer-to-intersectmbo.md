...
The Mithril repository is moving from the [`input-output-hk`](https://github.com/input-output-hk/mithril) GitHub organization to [`IntersectMBO`](https://github.com/IntersectMBO). The transfer is scheduled for **Tuesday, June 16, 2026 (D-day)**.
...
- Raw `githubusercontent.com` URLs for verification keys and configuration files.
...
- **At your next release**:
  ...
  - If your install scripts use `curl -s`, harden them to `curl -sSL` so HTTP redirects are followed.

#### Recommended `curl` flag

When fetching Mithril resources from raw GitHub URLs in scripts, use `-sSL` rather than `-s` so the request follows HTTP redirects and fails loudly on errors:

```bash
curl -sSL https://raw.githubusercontent.com/input-output-hk/mithril/main/networks.json
```
...
| :--: | --------------------------- | ------------- | ------------------------------------------------------------------------------------------- |
|  ✅  | Announcement (today)        | May 26, 2026  | This announcement is published                                                              |
|  ⬜  | D-day (repository transfer) | June 16, 2026 | Repository transfer is performed, new GHCR images are published under `IntersectMBO`        |
|  ⬜  | D+1                         | June 17, 2026 | Documentation cross-references and external references (e.g. Guild Operators) are refreshed |

The Mithril team will update this blog post on D-day to confirm that the transfer has been completed.
