...
impl MuSigSigner {
    ...
    fn partial_signature(self, pks: &[RistrettoPoint], committed_randomness: &[(RistrettoPoint, RistrettoPoint)], private_randomness: (Scalar, Scalar), message: &[u8]) -> MuSigSignature {
        ...
        let challenge = Scalar::from_hash(Blake2b::new()
            .chain(aggr_key.compress().as_bytes())
        ...
        MuSigSignature { announcement, response }
    }
    ...
    fn aggregate_signatures(sigs: &[MuSigSignature]) -> MuSigSignature {
        ...
        MuSigSignature { announcement: sigs[0].announcement, response: aggr_response }
    }
...
fn main() {
    ...
    let signers: Vec<MuSigSigner> = (0..nr_signers).into_iter().map(|_| MuSigSigner::new(&mut OsRng)).collect();
    let pks: Vec<RistrettoPoint> = signers.clone().into_iter().map(|signer| signer.pk).collect();

    let commitments: Vec<(Scalar, Scalar, RistrettoPoint, RistrettoPoint)> = (0..nr_signers).into_iter().map(|_| MuSigSigner::commit_randomness(&mut OsRng)).collect();
    let public_comms: Vec<(RistrettoPoint, RistrettoPoint)> = commitments.clone().into_iter().map(|comm| (comm.2, comm.3)).collect();

    let partial_signatures: Vec<MuSigSignature> = signers
        .into_iter()
        .enumerate()
        .map(|(index, signer)| signer.partial_signature(&pks, &public_comms, (commitments[index].0, commitments[index].clone().1), msg))
        .collect();
