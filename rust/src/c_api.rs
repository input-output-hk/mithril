...
mod initializer {
    ...
    #[no_mangle]
    pub extern "C" fn stm_initializer_verification_key(me: StmInitializerPtr, pk: *mut MspPkPtr) -> i64 {
        unsafe {
    ...
    pub extern "C" fn stm_initializer_new_signer(
        ...
        unsafe {
            if !me.is_null() && !closed_reg.is_null() {
                if let Some(ref_signer) = signer.as_mut() {
...
mod signer {
    ...
    pub extern "C" fn stm_signer_sign(
        ...
        unsafe {
            if let (Some(ref_me), Some(ref_out), Some(msg)) = (me.as_ref(), out.as_mut(), msg.as_ref()) {
                let msg_str = CStr::from_ptr(msg);
...
mod key_reg {
    ...
    pub extern "C" fn close_registration(
        ...
        unsafe {
            if let (Some(key_reg), Some(ref_closed_reg)) =
                (key_reg.as_mut(), closed_reg.as_mut())
            {
