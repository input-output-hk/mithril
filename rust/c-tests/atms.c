...
TEST(atm, produceAndVerifyAggregateSignature) {
    ...
    for (int i = 0; i < nr_signers; i++) {
        ASSERT_EQ(msp_generate_keypair(&sks[i], &keys[i]), 0);
    }
    ...
    for (int i = 0; i < 4; i++) {
         ASSERT_EQ(msp_sign(msg, sks[i], &sigs[i]), 0);
    }
    ...
    ASSERT_EQ(atms_aggregate_sigs(sigs, keys, avk_pk, 4, &aggregated_sig), 0);
...
TEST(atm, testingErrors) {
    ...
    ASSERT_EQ(atms_aggregate_sigs(sigs, keys, avk_pk, 2, &aggregated_sig_1), 0);
    ...
    for (int i = 0; i < 3; i++) {
        ASSERT_EQ(msp_generate_keypair(&fake_skeys[i], &fake_keys[i]), 0);
    }
    ...
    for (int i = 0; i < 2; i++) {
        ASSERT_EQ(msp_sign(msg, fake_skeys[i], &fake_sigs[i]), 0);
    }
    ...
    ASSERT_EQ(atms_aggregate_sigs(fake_sigs, fake_keys, avk_pk_fake, 2, &aggregated_sig_2), 0);
    ...
    MspPkPtr fake_key;
    MspSkPtr fake_skey;
    ASSERT_EQ(msp_generate_keypair(&fake_skey, &fake_key), 0);
    AvkPtr single_aggr_key;
    ASSERT_EQ(avk_key_aggregation(&fake_key, party_stake, 1, 1, &single_aggr_key), 0);
    MspSigPtr fake_sig;
    ASSERT_EQ(msp_sign(msg, fake_skey, &fake_sig), 0);

    AsigPtr aggregated_sig_3;
    ASSERT_EQ(atms_aggregate_sigs(&fake_sig, &fake_key, single_aggr_key, 1, &aggregated_sig_3), 0);
