...
use mithril::stm_batch_compat::StmInitializerBatchCompat;
    ...
    H: Digest + Clone + Sync + Send + FixedOutput,
{
    ...
    let mut ps: Vec<StmInitializerBatchCompat> = Vec::with_capacity(nparties);
    ...
    let signer = ps[0].clone().new_signer_batch_compat(closed_reg).unwrap();
