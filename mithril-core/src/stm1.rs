...
//!             .is_ok());
//!     }
//!     Err(AggregationError1::NotEnoughSignatures(n, k)) => {
...
use crate::error::{AggregationError1, RegisterError, StmSignatureError};
use crate::key_reg::ClosedKeyReg;
use crate::merkle_tree::{MTLeaf, MerkleTreeCommitment, Path, MerkleTree};
...
impl<D: Digest + Clone> StmClerk<D> {
    ...
    pub fn aggregate(
        ...
        sigs: &[StmSig<D>],
        msg: &[u8],
    ) -> Result<StmAggrSig<D>, AggregationError1<D>> {
        ...
        for sig in sigs{
            let path = self
                .closed_reg
                .merkle_tree
                .get_path(sig.mt_index.try_into().unwrap());
    ...
    pub fn dedup_sigs_for_indices(
        ...
        msg: &[u8],
        sigs: &[StmSig<D>],
    ) -> Result<Vec<StmSig<D>>, AggregationError1<D>> {
        ...
        Err(AggregationError1::NotEnoughSignatures(count, self.params.k))
    }
...
impl<D: Clone + Digest> StmSig<D> {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<StmSig<D>, StmSignatureError<D>> {
        ...
        u64_bytes.copy_from_slice(&bytes[160..]);
...
impl<D: Clone + Digest> StmAggrSig<D> {
    ...
    pub fn verify(
        ...
        // Check that all signatures did win the lotterys
    ...
    //         )?);
    //     }
    //     let mt = u64_bytes.copy_from_slice(&bytes[8..16]);
    //
    //     Ok(StmAggrSig { signatures, merkle_tree: mt })
    // }
}
...
mod tests {
    ...
    proptest! {
        ...
        fn test_aggregate_sig(nparties in 2_usize..30,
            ...
            match msig {
                ...
                Err(AggregationError1::NotEnoughSignatures(n, k)) =>
                    assert!(n < params.k || k == params.k),
                Err(AggregationError1::UsizeConversionInvalid) =>
                    unreachable!()
            }
        }
    }
    ...
    struct ProofTest {
        n: usize,
        msig: Result<Sig, AggregationError1<D>>,
        clerk: StmClerk<D>,
        msg: [u8; 16],
    }
