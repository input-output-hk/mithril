...
use super::{SnarkClerk, SnarkSetup, midnight_vk_serde};
...
impl<D: MembershipDigest> SnarkProof<D> {
    ...
    pub fn to_bytes(&self) -> StmResult<Vec<u8>> {
        ...
        buf.extend_from_slice(&self.snark_proof.len().to_le_bytes());
        buf.extend_from_slice(&self.snark_proof.clone());
        let mut buf_cvk = vec![];
        self.circuit_verification_key
            .write(&mut buf_cvk, SerdeFormat::RawBytes)?;
        buf.extend_from_slice(&buf_cvk);
        Ok(buf)
    }
...
mod tests {
    ...
    use crate::{
        ...
        proof_system::{
            SnarkClerk,
            halo2_snark::{SnarkSetup, proof::SnarkProof, unsafe_helpers::compute_circuit_degree},
        },
    };
    ...
    fn test_midnight_vk_serde() {
        ...
        let cvk = snark_setup.verification_key.clone();
        ...
        let mut buf_cvk = vec![];
        cvk.write(&mut buf_cvk, SerdeFormat::RawBytes).unwrap();

        let cvk_recover = MidnightVK::read(&mut buf_cvk.as_slice(), SerdeFormat::RawBytes).unwrap();

        snark_proof.circuit_verification_key = cvk_recover;
