...
/// The size of the proof generated by the certificate circuit
const SNARK_PROOF_LENGTH: usize = 3600;
/// Base degree of the circuit when the value k = 0. Can be used to infer
/// the circuit degree given the value k
// TODO: This constant can be removed or modified once the circuit is fixed
...
pub enum SnarkError {
    #[error(
        "The number of bytes used to create the proof is incorrect: received {0} instead of 3600."
    )]
...
impl SnarkProof {
    ...
    pub fn verify(
        ...
        let relation = StmCircuit::try_new(parameters, merkle_tree_depth).unwrap();
...
// TODO: This function should be removed when proper handling of the srs is created
#[allow(dead_code)]
...
mod test {
    ...
    use crate::{
        ...
        circuits::halo2::{
            circuit::StmCircuit,
            types::{CircuitMerkleTreeLeaf, MerklePath},
        },
    ...
    fn correct_proof_verifies() {
        ...
        let (closed_key_registration, signers) =
            prepare_key_registration_with_stakes(&params, stakes);
    ...
    fn random_proof_fails() {
        ...
        let stakes: Vec<u64> = (0..nb_signers).map(|_| random_range(10..100)).collect();
        let (closed_key_registration, _) = prepare_key_registration_with_stakes(&params, stakes);
        let avk: AggregateVerificationKeyForSnark<MithrilMembershipDigest> =
            AggregateVerificationKeyForSnark::from(&closed_key_registration);
        let merkle_tree_depth = nb_signers.next_power_of_two().trailing_zeros();
        let message = BaseFieldElement::from(42u64);
