...
fn next_epoch_step_proof_verifies() {
    // Asset-based check that the stored next-epoch Blake2b proof verifies
    // against the correct public inputs, confirming the existing step output
    // asset is valid and the verifier accepts a well-formed next-epoch
    // transition.
    let verification_context =
        load_embedded_verification_context_asset().expect("verification context asset should load");
    ...
    let public_inputs = [
        verification_context.global_field_elements.clone(),
        next_epoch_step_output.next_state.as_public_input(),
