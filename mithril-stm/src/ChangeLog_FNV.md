...
## Progress
### 2023-05-25
* `FullNodeSignature` struct, its implementation and related functions are removed.
  * The only difference between `FullNodeSignature` and `StmSig` was `FullNodeSignature` did not include the 
    `signer_index`. However, we need to provide index for full node verifier, so we decided to use `StmSig` as it is. 
* `StmSigner` is updated and it does not contain `closed_red` anymore. This signer is responsible for generating a 
  basic signature that does not depend on `avk`.
  * `StmSignerAvk` is a new structure that contains `StmSigner` and `closed_reg`, and it generates a signature 
    depending on `avk`.
* The implementation of `StmInitializer`:
  * `check_initializer`: checks whether the signer is actually registered and returns its index.
  * `new_signer`: Runs `check_initializer` and returns a `StmSigner`.
  * `new_signer_avk`: Runs `check_initializer` and returns a `StmSignerAvk`.
* The implementation of `StmSigner`:
  * `check_lottery`
  * `sign`
* The implementation of `StmSignerAvk`:
  * `sign`
* The implementation of `StmSig`:
  * `verify`
  * `verify_avk`
* The implementation of `FullNodeVerifier`:
  * The operations are still the same except that the parameter `&[FullNodeSignature, Index]` is changed to `&
    [StmSig] since we have index in `StmSig`.
