...
use pallas_network::{
    facades::DmqServer,
    miniprotocols::{
        localmsgsubmission::{DmqMsg, DmqMsgValidationError},
...
use mithril_common::{
    ...
    crypto_helper::{KesPeriod, KesVerifier, ProtocolKesSignature, ProtocolOpCert},
    ...
    async fn process_message(&self) -> StdResult<()> {
        ...
        let (dmq_message, response) = match request {
            Request::Submit(dmq_message) => {
                let dmq_message: StdResult<DmqMsg> = {
                    let kes_signature =
                        ProtocolKesSignature::from_bytes(&dmq_message.kes_signature)?;
                    let operational_certificate =
                        ProtocolOpCert::from_bytes(&dmq_message.operational_certificate)?;
                    let kes_period = dmq_message
                        .kes_period
                        .saturating_sub(operational_certificate.start_kes_period as KesPeriod);

                    self.kes_verifier.verify(
                        &dmq_message.msg_body,
                        &kes_signature,
                ...
                match dmq_message {
                    ...
                    Err(err) => {
                        error!(self.logger, "Failed to verify DMQ message"; "error" => ?err);
                        (
                            None,
                            Response::Rejected(DmqMsgValidationError(format!(
                                "Failed to verify DMQ message: {}",
                                err
                            ))),
                        )
                    }
...
mod tests {
    ...
    use mithril_common::{
        crypto_helper::{KesSignerFake, KesVerifierStandard},
        current_function,
        test_utils::TempDir,
    };

    use crate::{DmqMessageBuilder, test_tools::TestLogger};
    ...
    async fn fake_msg() -> DmqMsg {
        ...
        .set_ttl(100)
        .build(b"Test message for KES signing")
        .await
        .unwrap()
    }
