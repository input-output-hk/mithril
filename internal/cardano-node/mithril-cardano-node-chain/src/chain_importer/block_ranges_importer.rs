...
impl BlockRangeImporter {
    ...
    pub async fn run(&self, until: BlockNumber) -> StdResult<()> {
        ...
        for block_range in block_ranges {
            let blocks_and_transactions_mk_node = self
                .transaction_store
                .get_blocks_and_transactions_in_range(block_range.start..block_range.end)
                .await?;

            if blocks_and_transactions_mk_node.is_empty() {
                continue;
            }
...
mod tests {
    ...
    mod block_ranges {
        use mithril_common::entities::CardanoBlockTransactionMkTreeNode;

        use std::collections::BTreeSet;

        use super::*;

        pub fn merkle_root_for_blocks(block_ranges: &[CardanoBlockWithTransactions]) -> MKTreeNode {
            let nodes: BTreeSet<_> = block_ranges
                .iter()
        ...
        async fn if_nothing_stored_parse_and_store_all_block_ranges() {
            ...
            assert_eq!(
                ...
                repository.get_all_block_ranges().await
            );
        }
        ...
        #[tokio::test]
        async fn block_range_root_retrieves_only_strictly_required_blocks_and_transactions() {
            fn data_for_block(
                range: Range<BlockNumber>,
            ) -> StdResult<BTreeSet<CardanoBlockTransactionMkTreeNode>> {
                Ok(build_blocks(range.start, range.end - range.start)
                    .into_iter()
                    .flat_map(|b| b.into_mk_tree_node())
                    .collect())
            }
            ...
            let importer = {
                ...
                store_mock
                    ...
                    .once();
                store_mock
                    .expect_get_blocks_and_transactions_in_range()
    ...
    mod legacy_block_ranges {
        ...
        async fn if_nothing_stored_parse_and_store_all_block_ranges() {
            ...
            importer.run_legacy(up_to_block_number).await.unwrap();
        ...
        async fn if_all_block_ranges_computed_nothing_computed_and_stored() {
            ...
            importer.run_legacy(BlockNumber(10_000)).await.unwrap();
        ...
        async fn can_compute_block_ranges_up_to_the_strict_end_of_a_block_range() {
            ...
            importer.run_legacy(BlockRange::LENGTH * 2 - 1).await.unwrap();
        ...
        async fn can_compute_block_ranges_even_if_last_blocks_in_range_dont_have_transactions() {
            ...
            importer.run_legacy(BlockRange::LENGTH * 2).await.unwrap();
