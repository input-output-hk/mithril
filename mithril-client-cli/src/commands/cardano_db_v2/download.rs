...
impl CardanoDbV2DownloadCommand {
    ...
    pub async fn execute(&self, context: CommandContext) -> MithrilResult<()> {
        ...
        .with_context(|| {
            format!(
                "Can not get download and unpack cardano db snapshot for hash: '{}'",
                self.hash
            )
        })?;
    ...
    async fn compute_cardano_db_snapshot_message(
        ...
        .await
        .with_context(|| "Can not compute the cardano db snapshotmessage")?;
    ...
    fn log_download_information(
        ...
        if json_output {
            ...
            println!(
                r###"TODO Cardano db snapshot '{}' archives have been successfully unpacked. Immutable files have been successfully checked against Mithril multi-signature contained in the certificate.
