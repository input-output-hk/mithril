...
async fn main() -> MithrilResult<()> {
    ...
    let immutable_file_range = ImmutableFileRange::Range(1, 6);
    ...
    println!("Certificate message is {:?}", certificate.protocol_message);
    println!("Merkle root is {:?}", merkle_proof.root().to_hex());
    ...
    let message = wait_spinner(
        &progress_bar,
        MessageBuilder::new().compute_cardano_database_message(&certificate, &unpacked_dir),
    )
    .await?;
