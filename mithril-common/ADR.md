...
But we also want to document the rationale behind smaller decisions specific to this subproject, so they are not
lost in the shuffle, avoiding the need to rehash the same discussions in the future.
...
As of today, this serialization is only used by the `mithril-aggregator` to store the Artifacts in the database.
...
This will not affect the `mithril-aggregator` as it is not compiled to WebAssembly.
