...
/// to [MKTreeNode] without changing its `Into<MKTreeNode>` implementation.
/// The `Into<MKTreeNode>` implementation cover proof generation and certification for `CardanoTransactions`
...
impl MkSetProof<CardanoBlock> {
    /// Get the blocks certified by this proof
    pub fn blocks(&self) -> &[CardanoBlock] {
        &self.items
    }
...
impl MkSetProof<CardanoTransaction> {
    /// Get the transactions certified by this proof
    pub fn transactions(&self) -> &[CardanoTransaction] {
        &self.items
    }
