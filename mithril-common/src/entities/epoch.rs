...
impl Epoch {
    ...
    /// returns the genesis epoch instead of failing when the offset would yield a negative epoch
    /// (i.e. at the genesis epoch itself).
...
mod tests {
    ...
    #[test]
    fn offset_to_signer_retrieval_epoch_saturating_clamps_at_genesis() {
        assert_eq!(
            Epoch(2),
