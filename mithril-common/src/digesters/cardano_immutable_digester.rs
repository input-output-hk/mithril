...
impl ImmutableDigester for CardanoImmutableDigester {
    ...
    async fn compute_digests_for_range(
        ...
        let immutables_to_process = range.clone().try_fold(
            vec![],
            |mut vec: Vec<ImmutableFile>, immutable_file_number| {
                let immutable_files =
                    list_immutable_files_to_process(dirpath, immutable_file_number)?;
                immutable_files
                    .into_iter()
                    .for_each(|immutable_digest| vec.push(immutable_digest));

                Ok::<Vec<ImmutableFile>, ImmutableDigesterError>(vec)
            },
        )?;
