...
fn is_ledger_state_snapshot(path: &Path) -> bool {
    if path.is_dir() {
        path.join(IN_MEMORY_META).exists()
            && path.join(IN_MEMORY_STATE).exists()
            && path.join(IN_MEMORY_TABLES).exists()
            && path.join(IN_MEMORY_TABLES).join(IN_MEMORY_TVAR).exists()
    } else {
        path.is_file()
    }
}
    ...
    pub fn get_files_relative_path(&self) -> Vec<PathBuf> {
        match self {
            ...
            LedgerStateSnapshot::InMemory { folder_name, .. } => {
                vec![
                    PathBuf::from(folder_name).join(IN_MEMORY_META),
                    PathBuf::from(folder_name).join(IN_MEMORY_STATE),
...
mod tests {
    ...
    mod utxo_hd_in_memory_ledger_state {
        ...
        fn list_all_ledger_state_should_not_include_utxo_hd_folder_that_does_not_contains_meta_state_or_tvar_files(
            ...
            create_dir_all(ledger_with_missing_meta_files.join(IN_MEMORY_TABLES)).unwrap();
            create_fake_files(&ledger_with_missing_meta_files, &[IN_MEMORY_STATE]);
            create_fake_files(
                &ledger_with_missing_meta_files.join(IN_MEMORY_TABLES),
                &[IN_MEMORY_TVAR],
            );

            let ledger_with_missing_state_files = ledger_dir.join("200");
            create_dir_all(ledger_with_missing_state_files.join(IN_MEMORY_TABLES)).unwrap();
            create_fake_files(&ledger_with_missing_state_files, &[IN_MEMORY_META]);
            create_fake_files(
                &ledger_with_missing_meta_files.join(IN_MEMORY_TABLES),
                &[IN_MEMORY_TVAR],
            );
            ...
            create_dir_all(ledger_with_missing_tvar_files.join(IN_MEMORY_TABLES)).unwrap();
            create_fake_files(
                &ledger_with_missing_tvar_files,
                &[IN_MEMORY_STATE, IN_MEMORY_META],
            );
        ...
        fn list_all_ledger_state_with_valid_utxo_hd_folder_structure() {
            ...
            create_dir_all(ledger_state.join(IN_MEMORY_TABLES)).unwrap();
            create_fake_files(&ledger_state, &[IN_MEMORY_META, IN_MEMORY_STATE]);
            create_fake_files(&ledger_state.join(IN_MEMORY_TABLES), &[IN_MEMORY_TVAR]);
        ...
        fn get_relative_path_only_list_meta_state_and_tvar_files_even_if_there_are_other_files_in_the_folder(
            ...
            let target_dir = temp_dir_create!();
            create_dir_all(target_dir.join("050").join(IN_MEMORY_TABLES)).unwrap();
            ...
            assert_eq!(
                vec![
                    PathBuf::from("050").join(IN_MEMORY_META),
                    PathBuf::from("050").join(IN_MEMORY_STATE),
                    PathBuf::from("050")
                        .join(IN_MEMORY_TABLES)
                        .join(IN_MEMORY_TVAR)
                ],
