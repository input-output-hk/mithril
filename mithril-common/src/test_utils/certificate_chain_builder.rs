...
impl CertificateChainFixture {
    ...
    pub fn chain_reversed(&self) -> Vec<Certificate> {
        self.certificates_chained.iter().rev().cloned().collect()
    }

    /// Extract the chain starting from the certificate with the given hash and all it's parents
    ...
    mod certificate_chain_fixture {
        ...
        fn reversed_chain() {
            ...
            let expected: Vec<Certificate> = chain
                ...
                .rev()
                .collect();
            assert_eq!(chain.chain_reversed(), expected);
        }
    }
}
