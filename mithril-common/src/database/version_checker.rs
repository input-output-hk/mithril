...
use super::{ApplicationNodeType, VersionTracker, VersionTrackerProvider, VersionTrackerUpdater};
...
impl ApplicationVersionChecker {
    ...
    pub fn check(&self, current_semver: &str) -> Result<(), Box<dyn Error>> {
        ...
        let provider = VersionTrackerProvider::new(&connection);
        provider.create_table_if_not_exists()?;
        let updater = VersionTrackerUpdater::new(&connection);
        let version = provider
            .get_application_version(&self.application_type)?
            .unwrap_or_else(|| VersionTracker {
                app_semver: Version::from_str("0.0.0").unwrap(),
                application_type: self.application_type.clone(),
                migration_no: 0,
                updated_at: Local::now().naive_local(),
            });
        let current_semver = Version::parse(current_semver)?;

        match current_semver.cmp(&version.app_semver) {
            Ordering::Greater => {
                warn!(
                    &self.logger,
                    "Application version '{}' is out of date, new version is '{}'. Upgrading database…",
                    version.app_semver, current_semver
                );
                self.apply_migrations(&version, &current_semver, &updater, &connection)?;
                let current_version = updater.save(current_version)?;
                debug!(
                    &self.logger,
                    "database updated to version {}",
                    current_semver.to_string()
                );
            }
            Ordering::Less => {
                warn!(
