...
pub async fn csd_by_epoch(
    ...
    // Find the hash corresponding to the epoch
    let hash = csds
        .into_iter()
        .find(|csd| csd.epoch.to_string() == epoch)
        .map(|csd| csd.hash)
        .ok_or_else(|| {
            debug!("No stake distribution found for epoch={epoch}.");
            AppError::NotFound
        })?;
...
mod tests {
    ...
    async fn existing_csd_epoch() {
        ...
        let epoch: TmpEpochData = serde_json::from_str(csds.get(&hash).unwrap()).unwrap();
        let epoch = Path(epoch.epoch.to_string());
