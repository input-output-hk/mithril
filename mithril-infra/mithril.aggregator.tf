...
for SIGNER_PEER_PORT in $SIGNER_PEER_PORTS; do
  cat $AGGREGATOR_CONFIG_DIRECTORY/config/topology.json | jq '.localRoots[0].advertise = true' | jq '.localRoots[0].accessPoints += [{ "address": "${google_compute_address.mithril-external-address.address}", "port": '"$SIGNER_PEER_PORT"', "valency": 1}]' > $AGGREGATOR_CONFIG_DIRECTORY/config/topology.json.new
