...
# Setup dmq node topology
cat $SIGNER_TYPE_CONFIG_DIRECTORY/config/topology.json | jq 'del(.bootstrapPeers)' | jq '.localRoots[0].accessPoints[0] |= . + { "address": "${google_compute_address.mithril-external-address.address}", "port": ${local.mithril_aggregator_dmq_port}, "valency": 1}' > $SIGNER_TYPE_CONFIG_DIRECTORY/config/topology.json.new
...
EOT
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "export SIGNER_DMQ_ADDR='${google_compute_address.mithril-external-address.address}'",
